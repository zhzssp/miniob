...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) ||
      create_index.attribute_names.size() == 0) {
  ...
  /* 构建索引的各个字段的元数据 */
  int index_field_num = create_index.attribute_names.size();
  vector<const FieldMeta *> field_metas;
  field_metas.reserve(index_field_num);
  for(int i = 0; i < index_field_num; i++) {
      ...
      if(field_meta) {
        ...
        LOG_ERROR("Cannot get index's field %d", i);
      }
  }

  if (field_metas.size() < index_field_num) {
    LOG_WARN("Cannot get all field metas for index, db=%s, table=%s", 
             db->name(), table_name);
